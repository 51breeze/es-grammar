...
const Service = require('easescript/lib/service');
const provider = new Service({autoLoadDescribeFile:true});
var collection = null;
const setConfig=(document)=>{
    const config = vscode.workspace.getConfiguration('EaseScript',document);
    provider.setConfig({
        server:typeof config.Server === 'string' ? config.Server.split('|') : [],
        client:typeof config.Client === 'string' ? config.Client.split('|') : [],
        loadTypeFile:config.LoadTypeFile === void 0 ? true : !!config.LoadTypeFile,
    });
}
